import { roundTax, calculatePrice, generateReceipt } from "../sales";

describe("roundTax", () => {
  it("should round up to the nearest 0.05", () => {
    expect(roundTax(1.23)).toBe(1.25);
    expect(roundTax(2.1)).toBe(2.1);
    expect(roundTax(0.52)).toBe(0.55);
    expect(roundTax(3.49)).toBe(3.5);
  });
});
